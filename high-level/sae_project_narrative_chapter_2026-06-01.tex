...
Tier logs in \texttt{runs/tier\_rest} show repeated CUDA initialization failures that prevented completion of taxonomy, ViT transfer, and downstream dependent stages. Grid summaries in \texttt{runs/r20c10/grid\_summary.json} and \texttt{runs/seeds/grid\_summary.json} show 15 jobs each with zero successful result outputs under affected runs. R110 backbone training completed successfully (73.25\%, \texttt{runs/backbone\_r110\_c100/done.json}), but R110 reconstruction runs later failed when required cached activations were absent after interrupted cache stages.
...
A second limitation is robustness quantification under seed variation for some late-stage claims, especially where seed grids were blocked by runtime issues. A third limitation is generality beyond the core backbone where planned extensions (taxonomy comparisons and ViT transfer outputs) were implemented in code but blocked in execution on this environment.
...
The sixth decision point was operational rather than conceptual: tier campaign interruption under node/GPU instability. At this stage, some pending runs were blocked for infrastructure reasons despite implemented scripts and codepaths. The project therefore ended with uneven completeness: deep evidence on selected paths, explicit blocking logs on others.

Viewed end to end, this chronology is not random drift from plan. It is a sequence of evidence-conditioned reallocations: from feasibility to architecture discrimination, from architecture discrimination to causal-transition diagnostics, from diagnostics to mechanistic stabilization, and from mechanism to partially blocked generalization attempts.
