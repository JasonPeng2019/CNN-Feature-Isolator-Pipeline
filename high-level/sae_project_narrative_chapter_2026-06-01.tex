...
\subsection{Phase 3b changed: rollout-aligned raw chaining}
The changed Phase 3b experiment revisited the strongest version of the earlier claim that manifold projection was necessary at every hop. Instead of training only with re-grounded intermediate carriers, the updated run trained the predictor directly on raw predicted carriers while keeping the strongest stabilization tools that were already available: scheduled sampling, full-chain BPTT, gradient clipping, and per-step reconstruction/code losses.

This matters because it separates two hypotheses that the original Phase 3 story had partly entangled. One hypothesis was geometric: repeated raw transitions drift off the sparse manifold and therefore destroy useful structure. The second was procedural: the model fails because it is evaluated under rollout conditions that its training objective never adequately exposed it to. The changed result supports a mixed answer. Raw rollout remains weaker than the hybrid path, so geometric structure still matters, but the collapse is not absolute. Once training is aligned to the actual rollout setting, the raw chain becomes viable.

Concretely, the changed run in \texttt{runs/phase3b\_raw\_only\_changed/result.json} climbed from an epoch-0 raw chain of 0.0123 to a final raw chain of 0.6821. Evaluating the same trained model with re-grounding gave 0.6339, while the hybrid path remained best at 0.7120. The correct conclusion is therefore not that re-grounding was a mere patch, nor that it is strictly necessary in every regime. It is that re-grounding remains a strong practical stabilizer, while rollout-aligned raw-carrier retraining can recover much of the same failure mode that originally looked absolute.
...
At the chain-stability level, re-grounding transformed the narrative from binary failure to conditional success. Chain-aware training and BPTT-through-projection recovered most of the hybrid gap. The later changed raw-carrier retraining run then showed that part of the original raw-chain collapse was also a rollout-mismatch problem: when trained directly on raw predicted carriers, the chain recovered to 0.6821 even without re-grounding in the carrier path. The residual gap to the 0.7120 hybrid path likely reflects both persistent hard-edge mismatch (especially around Q3$\rightarrow$Q4 geometry changes) and remaining mismatch between predictor families and true inter-section dynamics.

In short, the project confirmed sparse sufficiency and revealed a precise compositional constraint: sparse-code chains are fragile under naive recursive rollout, but they become usable when either repeatedly projected back onto destination SAE manifolds or trained directly under rollout-aligned carrier conditions. In the current artifact record, hybrid/re-grounded transport still remains the strongest practical path, but the raw-chain failure story is no longer absolute.

\subsection{Changed follow-ups on fairness and patchwise baselines}
The changed follow-up suite also sharpened two supporting interpretations. First, the fair-budget receptive-field rerun completed successfully across all 15 planned cells and reached a best top-1 of about 0.7173. This weakens the earlier suspicion that receptive-field locality itself was intrinsically too restrictive; much of the older weakness came from an unfair budgeting scheme rather than locality alone.

Second, the patchwise SAE retries clarified the old vector-coder failure mode. The overlapping patchwise sweep completed across all 45 planned cells and showed that the strongest settings were usually the smallest CNN-filter-scaled patches rather than larger ones, especially in earlier quarters. The depth-aware patch sweep produced only a partial artifact record (25 completed cells present rather than the full planned sweep), and the disjoint-patch experiment does not currently have an output directory in the repo. So the patchwise baseline story is improved but not closed: overlapping local patch context helps, larger patches are not automatically better, and the disjoint-patch hypothesis remains untested in the current artifact tree.
...
This progression provides a coherent causal interpretation, but the later changed rerun adds one more layer to it. The dominant bottleneck in the original multi-step sparse-code chaining setup was not lack of sparse information and not merely insufficient predictor capacity; it was recursive off-manifold drift coupled with train-test mismatch. Re-grounding is effective because it directly targets that drift at each composition boundary. But the changed raw-carrier retraining result shows that some of the same instability can also be reduced by training the predictor on its own rollout distribution rather than only on teacher-like carriers.
