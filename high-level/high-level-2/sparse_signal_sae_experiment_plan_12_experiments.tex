...
\subsection{Fair-budget comparison note (recommended addition)}

If Global masking is tuned by percentage/fraction retained while RF-local masking is tuned only by a fixed \(K_{\mathrm{RF}}\), the two settings may operate at very different effective sparsity levels across sections. This can make Global vs RF comparisons unfair, especially near sections where receptive-field geometry changes.

For fair comparison experiments, add an RF-percentage variant:
\[
\text{retain top } p_{\mathrm{RF}}\% \text{ of coefficients inside each RF window}
\]
or adapt \(K_{\mathrm{RF}}\) per section so that RF-local masking matches a target effective retained fraction.

This is particularly important at late sections (for example \(Q_5\)) where fixed-\(K_{\mathrm{RF}}\) masking can become degenerate and retain an extremely small global fraction by construction.
...
\section{Training Protocols: Three Distinct Families}

The experiment grid must distinguish three different protocols. These are not interchangeable.

\begin{enumerate}[leftmargin=*]
    \item \textbf{Full independent reconstruction only.} Each section \(Q_t\) gets its own independently trained SAE. There is no cross-section chain. This is the baseline that asks whether each hidden map can be reconstructed from a sparse masked transform field.
    \item \textbf{Independent-SAE chain.} Each section \(Q_t\) still gets its own independently trained SAE, but after those SAEs are trained, learn a transition from the masked sparse code at \(Q_t\) to the independently learned sparse code at \(Q_{t+1}\). This is the chain where every target code is independently grounded first.
    \item \textbf{Dependent/incremental chain.} The sparse code at \(Q_{t+1}\) is not first learned independently. Instead, it is learned from the sparse code at \(Q_t\) while still reconstructing the true hidden state at \(Q_{t+1}\). This is the stronger sparsity-tree protocol.
\end{enumerate}
...
\section{Protocol A: Full Independent Layerwise Reconstruction}

\subsection{Goal}

This protocol trains each section's SAE independently and stops there. It does not learn any chain between sections.

The goal is to establish whether each hidden map \(\Htens_t\) can be represented and reconstructed from a sparse masked coefficient field:
\[
\Htens_t
\rightarrow
E_t(\Htens_t)=\Ztens_t
...
\section{Protocol B: Independent-SAE Chain}
...
\[
\Ztilde_t \rightarrow \Zhat_{t+1}.
\]

The target \(\Ztens_{t+1}\) is the independently learned code produced by \(E_{t+1}\). This means every target space is independently grounded in the true hidden map before any transition is learned.

\subsection{Stage B.1: Train all layerwise SAEs independently}
...
\subsection{Stage B.2: Learn sparse transitions between independent spaces}
...
\subsection{Stage B.3: Chain from \(Q_1\) to \(Q_5\)}
...
\section{Protocol C: Dependent / Incremental Sparse-Code Chain}
...
\subsection{Transition module choices}
...
\begin{itemize}[leftmargin=*]
    ...
    \item a frozen original-model path: decode \(\Ztilde_t\), run the original CNN blocks from \(Q_t\) to \(Q_{t+1}\), then encode or map into the next sparse code;
    \item a hybrid transition that mixes learned sparse-code prediction with decoded hidden-state reconstruction.
\end{itemize}
...
\section{The Twelve Experiments}
...
\begin{enumerate}[leftmargin=*]
    ...
    \begin{itemize}
        \item Full independent layerwise reconstruction only.
        \item Independent-SAE chain.
        \item Dependent/incremental sparse-code chain.
    \end{itemize}
...
\begin{longtable}{p{0.055\textwidth}p{0.24\textwidth}p{0.24\textwidth}p{0.19\textwidth}p{0.19\textwidth}}
\caption{Twelve core experiments.}\\
\toprule
...
E3 & Full independent reconstruction only & Shared vector / patchwise SAE & Global mask & Can the safest standard SAE independently reconstruct each section from globally sparse coefficient fields?\\
\addlinespace

E4 & Full independent reconstruction only & Shared vector / patchwise SAE & Receptive-field mask & Can shared feature-by-patch codes reconstruct each section under RF-local sparsity?\\
\addlinespace
...
E7 & Independent-SAE chain & Shared vector / patchwise SAE & Global mask & Can standard shared SAE codes predict the next independently learned sparse code under global masking?\\
\addlinespace

E8 & Independent-SAE chain & Shared vector / patchwise SAE & Receptive-field mask & Can standard shared feature-by-patch codes form a local sparse chain under RF masks?\\
\addlinespace
...
E11 & Dependent/incremental sparse-code chain & Shared vector / patchwise SAE & Global mask & Can a standard shared SAE support dependent sparse-code learning under global compression?\\
\addlinespace

E12 & Dependent/incremental sparse-code chain & Shared vector / patchwise SAE & Receptive-field mask & Can the safest SAE plus RF-local masking learn an incremental sparse hierarchy?\\
