...
One later changed follow-up also updates the interpretation of Phase 3b. A rollout-aligned raw-carrier retraining run completed successfully in \texttt{runs/phase3b\_raw\_only\_changed/result.json} and showed that raw chaining is not inherently doomed once the predictor is trained directly under its own rollout conditions. This does not overturn the practical value of re-grounding, but it softens the earlier stronger claim that repeated manifold projection was strictly necessary.
...
\subsection{Phase 3b changed: raw-carrier retraining without re-grounding}
The changed Phase 3b rerun directly tested whether the previous raw-chain collapse was an unavoidable property of the representation, or whether it partly reflected a rollout mismatch between training and evaluation. In this version, the predictor was trained on raw predicted carriers rather than on a re-grounded carrier path, while keeping scheduled sampling, full-chain BPTT, gradient clipping, and the usual per-step reconstruction/code losses.

This run completed successfully and produced a clear change in behavior. The raw chain started near chance (0.0123 at epoch 0) but climbed to a final 0.6821. Evaluating the same trained model with re-grounding gave 0.6339, while the hybrid path remained strongest at 0.7120. The main conclusion is that direct rollout-aligned retraining recovers most of the raw-chain collapse. Thus, re-grounding remains an effective stabilizer but is no longer the only viable mechanism supported by the artifact record.
...
\subsection{Changed RF and patchwise follow-ups}
The changed follow-up suite also sharpened two supporting interpretations. First, the fair-budget receptive-field rerun completed successfully across all 15 planned cells and reached a best top-1 of about 0.7173, showing that receptive-field locality remains competitive when the retained-code budget is normalized fairly within each RF window. This weakens any earlier claim that locality itself was the main reason RF-style masking lagged.

Second, the patchwise SAE retries clarified the old vector-coder failure mode. The overlapping patchwise sweep completed across all 45 planned cells and showed that the strongest settings were usually the smallest CNN-filter-based patches rather than larger ones, especially in earlier quarters. The depth-aware patch sweep produced only a partial artifact set (25 cells present rather than the full planned sweep), with early evidence favoring the less aggressive schedule that stays near the base filter size. The disjoint-patch experiment does not currently have an output directory in the repo, so it should be treated as not yet executed in the artifact record rather than as a negative result.
...
These limitations constrain claim scope. Strong claims are justified for executed winner-path mechanisms, Family I matrix outcomes, the changed raw-carrier retraining result, and the completed extension runs named above. Broad claims over all planned Family II architecture/mask combinations are still not justified, the changed patchwise follow-up suite is incomplete, and the transfer story beyond the CNN winner path is still much thinner than the core reconstruction and chain evidence.
...
The changed Phase 3b rerun makes that conclusion more precise. Transition stabilization can come from explicit manifold projection, from rollout-aligned retraining, or from a combination of both. In the current artifact record, hybrid/re-grounded paths still win operationally, but the raw-chain failure story is no longer absolute.
